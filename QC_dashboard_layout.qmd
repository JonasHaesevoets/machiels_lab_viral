---
title: "QC Experiment 1, day 60, library: LUNG"
format: 
  dashboard:
    orientation: columns
...
```{r}
#| title: nCount marginal filtered

multiplet_n_count_RNA_n_Feature_filtered<- filtered_seurat_obj |>  ggplot(aes(nCount_RNA, nFeature_RNA, color=seurat_clusters)) + 
  geom_point(size=0.5) +
  theme_bw() +
  xlim(c(-100,70000)) +
  ylim(c(-1000,8000)) +
...
```{r, cluster_0_feature_umap, eval= eval_0}
#| title: cluster 0
feature_umap(clus_1_genes, title_= title_feature_umap_clus_1, caption_ = caption_feature_umap_clus_1) 
```

```{r, cluster_1_feature_umap, eval= eval_1}
#| title: cluster 1
feature_umap(clus_1_genes,title_feature_umap_clus_1, caption=caption_feature_umap_clus_1)
```

```{r, cluster_2_feature_umap, eval= eval_2}
#| title: cluster 2
feature_umap(clus_2_genes,title_feature_umap_clus_2, caption=caption_feature_umap_clus_2)
```

```{r, cluster_3_feature_umap, eval= eval_3}
#| title: cluster 3
feature_umap(clus_3_genes,title_feature_umap_clus_3, caption=caption_feature_umap_clus_3)
```

```{r, cluster_4_feature_umap, eval= eval_4}
#| title: cluster 4
feature_umap(clus_4_genes,title_feature_umap_clus_4, caption=caption_feature_umap_clus_4)
```

```{r, cluster_5_feature_umap, eval= eval_5}
...
```{r, cluster_14_feature_umap, eval= eval_14}
...
```{r, cluster_15_feature_umap, eval= eval_15}
...
```{r, cluster_16_feature_umap, eval= eval_16}
#| title: cluster 16
...
```{r, cluster_21_feature_umap, eval= eval_21}
#| title: cluster 21
...
# cluster 0 - cause of undetermined
...
### Row {.tabset}
```{r}
#| title: Features cluster 0
feature_umap(clus_0_genes) +
...
```{r}
#| title: nCount RNA -demuliplex groups 
seurat_obj |> filter(RNA_snn_res.0.5==0) |> ggplot(aes(nCount_RNA, sampletag_multiplets,fill=sampletag_multiplets)) + geom_density_ridges()+labs(title = "cluster 0 only",caption="undetermined not low in RNA")
```
```{r}
#| title: nFeature RNA -demuliplex groups 
seurat_obj |> filter(RNA_snn_res.0.5==0) |> ggplot(aes(nFeature_RNA, sampletag_multiplets,fill=sampletag_multiplets)) + geom_density_ridges() +labs(title = "cluster 0 only", caption="undetermined not low in complexity")
```
### Row {.tabset}
...
```
```{r}
#| title: rhapsody dehash multiplet
DimPlot(cluster_0, group.by = "sampletag_multiplets")
```
```{r}
#| title: umap based grouping
