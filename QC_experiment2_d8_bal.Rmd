...
file_names_tibble <- tribble(
    ...
    "bal",
    #library_name
    paste0(file_path$intermediate_data,"seurat_obj_experiment_2_combined_bal_raw_dbec.rds"),
    #raw_seurat_obj_path
    paste0(file_path$raw_data_root,"output_bal_d8/","BD-Analysis-BMachiels-BAL_Sample_Tag_Calls.csv"),
    #path_sample_tag_calls
    paste0(file_path$intermediate_data,"seurat_obj_experiment_2_bal_workflowed.rds"),
    #write_processed_seurat_obj_path
)
```
...
```{r, echo=FALSE}
seurat_d8_bal_raw = read_rds(file_names_tibble$raw_seurat_obj_path)
...
seurat_d8_bal_raw = seurat_sampletag_name(seurat_obj = seurat_d8_bal_raw, sample_tag_calls = sample_tag_calls)
seurat_d8_bal_raw$amountUMI = seurat_d8_bal_raw$nCount_RNA
seurat_d8_bal_raw$amountGenes = seurat_d8_bal_raw$nFeature_RNA
```
...
```{r, echo=FALSE}
seurat_d8_bal_raw$log10GenesPerUMI = log10(seurat_d8_bal_raw$nFeature_RNA) / log10(seurat_d8_bal_raw$nCount_RNA)
...
```{r, echo=FALSE}
seurat_d8_bal_raw$mitoRatio <- PercentageFeatureSet(object = seurat_d8_bal_raw, pattern = "^mt-")
seurat_d8_bal_raw$mitoRatio <- seurat_d8_bal_raw@meta.data$mitoRatio / 100
```
...
```{r, echo=FALSE}
seurat_d8_bal_raw = dplyr::filter(seurat_d8_bal_raw, sampletag_multiplets == "single_hashtag")
seurat_d8_bal_raw
```

### after filtering for singlets only 10594 cells out of 11500 were retained
...
```{r}
seurat_d8_bal_filtered = dplyr::filter(seurat_d8_bal_raw, 
                                (mitoRatio < 0.2) &
                                (amountGenes > 1200) &
                                (amountUMI > 5000) &
...
# 8219 out of 11500 cells were retained
...
```{r}
countsDataset <- GetAssayData(object = seurat_d8_bal_filtered, slot = "counts")
...
seurat_d8_bal_filtered1 <- CreateSeuratObject(counts = filtered_counts, meta.data = seurat_d8_bal_filtered@meta.data)
# copy them back from the object where the genes werent filtered
seurat_d8_bal_filtered1@assays$protein = seurat_d8_bal_filtered@assays$protein
seurat_d8_bal_filtered1@assays$sampletags = seurat_d8_bal_filtered@assays$sampletags
```

# After QC we retain 21592 genes and 8219 cells from 27314 genes and 11500 cells
...
```{r}
seurat_d8_bal_filtered_normalized = NormalizeData(seurat_d8_bal_filtered1)
```
...
seurat_d8_bal_filtered_normalized <- CellCycleScoring(seurat_d8_bal_filtered_normalized,
...
#| fig-width: 10
#| fig-height: 8
VlnPlot(seurat_d8_bal_filtered_normalized, features = c("S.Score", "G2M.Score"), group.by = "sampletag_name")
```
...
```{r}
seurat_d8_bal_filtered_normalized <- FindVariableFeatures(seurat_d8_bal_filtered_normalized, 
                     selection.method = "vst",
                     nfeatures = 2000, 
                     verbose = FALSE)
seurat_d8_bal_filtered_normalized = ScaleData(seurat_d8_bal_filtered_normalized)
seurat_d8_bal_filtered_normalized = RunPCA(seurat_d8_bal_filtered_normalized)
...
#| fig-width: 10
#| fig-height: 8
DimPlot(seurat_d8_bal_filtered_normalized,
        reduction = "pca",
        group.by= "Phase",
        split.by = "Phase")
```
...
```{r}
seurat_d8_bal_filtered_normalized <- ScaleData(seurat_d8_bal_filtered_normalized, vars.to.regress = c("S.Score", "G2M.Score"))
...
seurat_d8_bal_filtered_normalized$gender <- seurat_d8_bal_filtered_normalized |> 
