---
title: "DeSeq2_exp1_all_conditions"
format:
    html:
      code-fold: true
...
```{r}
bal_alv_mac_wide_sex |> ggplot(aes(Eif2s3y, sampletag_name, fill=sampletag_name)) + geom_density_ridges()+ggtitle("Eif2s3y per sample")
```


```{r}
bal_alv_mac_wide_sex |> ggplot(aes(Kdm5d, sampletag_name, fill=sampletag_name)) + geom_density_ridges()+ggtitle("Kdm5d per sample")
```


```{r}
bal_alv_mac_wide_sex |> ggplot(aes(max_male_genes, sampletag_name, fill=sampletag_name)) + geom_density_ridges() +ggtitle("max. male gene")
```
...
plotCounts(res, gene="Spp1", intgroup="virus", 
                returnData=TRUE) |> expression_plotter()+ggtitle("Spp1")
```
```{r}
plotCounts(res, gene="Fyb", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Fyb")
```

```{r}
plotCounts(res, gene="Scd2", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Scd2")

```
```{r}
plotCounts(res, gene="Scd1", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Scd1")

```

```{r}

plotCounts(res, gene="Serpinb6a", intgroup="virus", 
...
```{r}
plotCounts(res, gene="Fabp4", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Fabp4")
```
```{r}
plotCounts(res, gene="Ccl9", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Ccl9")
```
```{r}
plotCounts(res, gene="Cd93", intgroup="virus", 
...
```{r}
plotCounts(res, gene="Mmp12", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Mmp12")
```
```{r}
plotCounts(res, gene="Ecm1", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Ecm1")
```
```{r}
plotCounts(res, gene="Wfdc17", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Wfdc17")
```
...
```
```{r}
plotCounts(res, gene="Egfem1", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Egfem1")
```
```{r}
plotCounts(res, gene="Nav2", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Nav2")
```

```{r}
plotCounts(res, gene="Apoc1", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Apoc1")
```
...
plotCounts(res, gene="Nt5e", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Nt5e")
```


```{r}
plotCounts(res, gene="Cd74", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Cd74")
```
...
```{r}
results(res, contrast=list("Ms4a3Ms4a3_pos.virusPVM", "Ms4a3Ms4a3_pos.virusPR8"))
```
...
dat <- res_all |>
 ...
 dat |>  ggplot(aes(label=gene, x=x_pos,y=y_pos, color=log2FoldChange, size=neg_log_padj))+
  ...
  xlab("reference")+ scale_size(range = c(3, 5))

```

```{r}
res_all |> mutate(ref_contr=paste(reference,contrast,sep = "_")) |> ggplot(aes(pvalue))+geom_histogram()+facet_wrap(~reference)
```
...
for (i in seq_along(contrast_names)) {
  ...
  scale_color_gradient2()+ggtitle(paste("reference:",contrast_names[i], "vs.",contrast_names[j] ))
...
res_all %>% filter(padj<0.9) 
```



```{r}
plotCounts(res, gene="Ccnd1", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Ccnd1")
```
```{r}
plotCounts(res, gene="Socs3", intgroup="virus", 
                returnData=TRUE)|> expression_plotter()+ggtitle("Socs3")
```


```{r}
res_all |> ggplot(aes(pvalue))+geom_histogram()+facet_wrap(~nominator)
```
