...
nFeature RNA = #genes detected / cell =amountGenes

```{r, echo=FALSE}

seurat_d60_lung_raw = seurat_sampletag_name(seurat_obj = seurat_d60_lung_raw, sample_tag_calls = sample_tag_calls)
seurat_d60_lung_raw$amountUMI = seurat_d60_lung_raw$nCount_RNA
seurat_d60_lung_raw$amountGenes = seurat_d60_lung_raw$nFeature_RNA
```
...
```{r, echo=FALSE}
seurat_d60_lung_raw = dplyr::filter(seurat_d60_lung_raw, sampletag_multiplets == "single_hashtag")
seurat_d60_lung_raw
```
...
```{r}
seurat_d60_lung_filtered = dplyr::filter(seurat_d60_lung_raw, 
                                (mitoRatio < 0.2) &
                                (amountGenes > 1200) &
                                (amountUMI > 5000) &
...
# 9833 out of 14000 cells were retained
...
```{r}
countsDataset <- GetAssayData(object = seurat_d60_lung_filtered, slot = "counts")
...
seurat_d60_lung_filtered1 <- CreateSeuratObject(counts = filtered_counts, meta.data = seurat_d60_lung_filtered@meta.data)
# copy them back from the object where the genes werent filtered
seurat_d60_lung_filtered1@assays$protein = seurat_d60_lung_filtered@assays$protein
seurat_d60_lung_filtered1@assays$sampletags = seurat_d60_lung_filtered@assays$sampletags
```

# After QC we retain 21827 genes and 14228 cells from 27314 genes and 18000 cells
...
```{r}
seurat_d60_lung_filtered_normalized = NormalizeData(seurat_d60_lung_filtered1)
```
...
seurat_d60_lung_filtered_normalized <- CellCycleScoring(seurat_d60_lung_filtered_normalized,
...
#| fig-width: 10
#| fig-height: 8
VlnPlot(seurat_d60_lung_filtered_normalized, features = c("S.Score", "G2M.Score"), group.by = "sampletag_name")
```

The violin plot shows clear evidence (altough a bit less pronounced then in d60 bal) of a substantial amount of cycling genes in this case. Further investigate this with a PCA plot
...
```{r}
seurat_d60_lung_filtered_normalized <- FindVariableFeatures(seurat_d60_lung_filtered_normalized, 
                     selection.method = "vst",
                     nfeatures = 2000, 
                     verbose = FALSE)
seurat_d60_lung_filtered_normalized = ScaleData(seurat_d60_lung_filtered_normalized)
seurat_d60_lung_filtered_normalized = RunPCA(seurat_d60_lung_filtered_normalized)
...
#| fig-width: 10
#| fig-height: 8
DimPlot(seurat_d60_lung_filtered_normalized,
        reduction = "pca",
        group.by= "Phase",
        split.by = "Phase")
```
...
# effect of cell cycle, less pronounced then in d60 bal

```{r}
seurat_d60_lung_filtered_normalized |> write_rds("intermediate_data/seurat_obj_experiment_1_lung_afterQCdashboard.rds")
```
## 6.3 Check for effect of sex

```{r}
y_chromosome_genes<-   c("Ddx3y", "Eif2s3y", "Kdm5d")
seurat_d60_lung_filtered_normalized$gender <- seurat_d60_lung_filtered_normalized |> 
