...
libraries("Seurat", "tidyverse", "tidyseurat","ggExtra", "viridis", "ggridges","patchwork", "ggpubr","dsb","Matrix", "ggnewscale"
)
```
...
protein_dsb |>filter(method=="dsb") |>  ggplot(aes(orig.ident, value)) +geom_boxplot()+ facet_wrap(~ marker) + theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust=1)) +ggtitle(" corrected not scaled by library")
...
```
Max-scaling performs reasonably well. However Siglecf in experiment 2-lung only has a very small proportion of cells that would count as postive.
 With 95th quantile scaling Siglecf in the same sample is present in a continuous distribution. This has to be checked for biological soundness.
 Ly6g and Ly6C do not possess a resonably sized positve fraction in max-scaling. 90th quantile scaling results in a gradual distribution.
...
90th quantile scaling results in a bimodal ditribution of Ly6G in cluster 10, which is unlikly to be biologically sound.
All clusters, including cluster 1, appear to be Ly6A-negative in max. scaling. 
A positive shoulder appears for Ly6a in 90th quantile scaling. Therefore Ly6a-90th-quantile scaled will be kept in the meta data for later analysis.
...
Hence, negative values  represent statistical variation of values close to zero. These are though just zero. Max scaling will amplify this noise signal, possibly resulting in spurious signal. Therfore negative values will be adjusted to zero.
...
protein_dsb |>
  ...
  geom_density_ridges()+facet_wrap(~marker)
```


# no-zero dsb corrected max scaled adt counts
...
group_by(.feature,orig.ident,condition,sampletag_Ms4a3) |>
  summarise(mean_scaled_dsb=mean(.abundance_dsb)) |>
  group_by(.feature) |> 
...
obj.v5 |>
  join_features(c("Siglecf-AbSeq", "H2-ia-ie-AbSeq", "Cd274-AbSeq", "Cd11c", "Ly-6g-AbSeq", "Ly-6a-AbSeq"
                  ),assay="dsb", slot="data") |>  as_tibble() |> separate(orig.ident, sep = "__", into = c("day", "sample_type"), remove = FALSE) |> 
    mutate(day=str_replace_all(day,c("viral.experiment.1"="d60",
                                                 "viral.experiment.2"="d8") )) |>
  relocate(day) |>
  group_by(.feature,day,sample_type, condition,sampletag_Ms4a3) |>
  summarise(mean_scaled_dsb=mean(.abundance_dsb)) |>
  group_by(.feature) |> 
  mutate(zscore_mean_scaled_dsb=(mean_scaled_dsb-mean(mean_scaled_dsb))/sd(mean_scaled_dsb))|> na.omit() |> 
  mutate(zscore_mean_scaled_dsb=ifelse(zscore_mean_scaled_dsb>2,2,
                                       ifelse(zscore_mean_scaled_dsb<(-2),-2,zscore_mean_scaled_dsb))) |> 
  ggplot(aes(.feature,condition, fill=zscore_mean_scaled_dsb)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  ggh4x::facet_nested(day~sample_type + sampletag_Ms4a3)
```
```{r}
# obj.v5 |>  as_tibble() |>
...
#          ) |> 
#   group_by(day,sample_type, condition,sampletag_Ms4a3) |>
#   summarize(n_total=n()#, .before=.cell
#             ) |> 
#   group_by(day,sample_type, condition,sampletag_Ms4a3, harmony_cluster_8dims_rough) |>
#   summarize(freq_cluster=n()/n_total) |> unique() |> 
#   ggplot(aes(harmony_cluster_8dims_rough,condition, fill=freq_cluster)) +
#   geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
#   ggh4x::facet_nested(day~sample_type + sampletag_Ms4a3)
...
obj.v5 |>  as_tibble() |>
  ...
  mutate(freq_cluster=n/sum(n)) |> 
  ggplot(aes(condition,harmony_cluster_8dims_rough, fill=freq_cluster)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  ggh4x::facet_nested(day~sample_type + sampletag_Ms4a3)
...
obj.v5 |>  as_tibble() |>
  ...
  ggplot(aes(condition,harmony_cluster_8dims_rough, fill=freq_cluster)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
  ggh4x::facet_nested(~sampletag_Ms4a3+sample_type +day)
...
obj.v5 |>  as_tibble() |>
  ...
  ggh4x::facet_nested(~sampletag_Ms4a3+sample_type +day) +theme_bw()+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))
...
obj.v5 |>  as_tibble() |>
  ...
  geom_tile()+ scale_fill_viridis()+theme_bw() +
  ggh4x::facet_nested(~sampletag_Ms4a3+sample_type +day) +theme_dark()+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +
...
```{r}
obj.v5 |> mutate(experiment=str_replace_all(orig.ident,
                                            c("viral.experiment.1_"="d60",
                                              "viral.experiment.2_"="d8"))) |>
  group_by(experiment,sampletag_Ms4a3, condition) |> 
  mutate(n_experiment=n()) |>
  relocate(n_experiment, experiment)
...
```{r}
obj.v5|> DimPlot(label=T, reduction = "umap.harmony_8dims")
```
