...
libraries("Seurat", "tidyverse", "tidyseurat","ggExtra", "viridis", "ggridges","patchwork", "ggpubr","dsb","Matrix", "ggnewscale", "data.table")
```
...
for (i in seq_along(unfiltered_csv_paths)) {
  ...
  #sqve unfiltered_prot_counts  as intermediate data
...
for (i in seq_along(unfiltered_prot_counts)) {
  ...
  protein_sum_tbl <- tibble(cell_sum=protein_counts_cell_is_col |>
                              colSums(), cell=colnames(protein_counts_cell_is_c ol))
  ...
  ggsave(plot=plot_output_1, filename = paste0("../../R_projects/machiels_lab_viral/output/plots/","fread_protein_BD_empty_cell_comparison_" ,dataset_name[i],".png") )
  
  ggsave(plot=plot_output_2, filename =paste0("../../R_projects/machiels_lab_viral/output/plots/","fread_RNA_comparison_BD_empty_cell_" ,dataset_name[i],".png") )
...
Max-scaling performs reasonably well. However Siglecf in experiment 2-lung only has a very small proportion of cells that would count as postive. With 95th quantile scaling Siglecf in the same sample is present in a continuous distribution. This has to be checked for biological soundness. Ly6g and Ly6C do not possess a resonably sized positve fraction in max-scaling. 90th quantile scaling results in a gradual distribution.
