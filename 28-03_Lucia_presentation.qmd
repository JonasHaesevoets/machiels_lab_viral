...
```{r}
DimPlot_scCustom(seurat_d60_1_alveolar, split.by = "condition", group.by = "sample_type",colors_use = c("blue", "orange"))
...
```{r}
DimPlot_scCustom(seurat_d60_1_alveolar, split.by = "condition", group.by = "sampletag_Ms4a3", colors_use = c("darkgrey", "red") )
...
Idents(seurat_d60_1_alveolar) = "sampletag_name_factor"
DimPlot_scCustom(seurat_d60_1_alveolar, split.by = "sampletag_name_factor", num_columns = 10, colors_use = c("red", "orange", "black", "darkgrey", "blue", "cyan", "green", "lightgreen", "purple", "pink"), label = F)
...
dimplot_Ms4a3 = DimPlot_scCustom(seurat_d60_1_alveolar, group.by = "sampletag_Ms4a3",  colors_use = c("darkgrey", "red"))
dimplot_Ms4a3
...
DimPlot_All_Samples(seurat_d60_1_alveolar, meta_data_column = 'sampletag_name', colors_use = pallet, num_columns = 10)
...
```{r}
Idents(seurat_d60_1_alveolar) = "seurat_clusters"
...
for (i in markers_am) {
  plot <- FeaturePlot_scCustom(seurat_d60_1_alveolar,  features = i, label = FALSE, label.size = 5,  split.by = "sampletag_Ms4a3", colors_use = pallet, label_feature_yaxis = FALSE )
...
create_and_save_plot <- function(gene) {
  plot <- FeaturePlot_scCustom(seurat_d60_1_alveolar, 
                              features = gene, 
                              label = TRUE, 
                              label.size = 5, 
...
plot_cd11c = FeaturePlot_scCustom(obj.v5, features = c("Cd11c"), colors_use = pallet)
...
plot_siglecf = FeaturePlot_scCustom(obj.v5, features = c("Siglecf-AbSeq"), colors_use = pallet)
plot_siglecf + scale_fill_continuous(limits = c(0, 15)) +
  scale_color_continuous(limits = c(0, 15)) +
  scale_size_continuous(limits = c(0, 15))
...
```{r}
top_d60_alveolar5 <- select_top_genes(markers_d60_alveolar_filtered, n = 10)

genes_d60_alveolar = top_d60_alveolar5$gene
...
# Assuming top_d60_alveolar5 is available (replace with actual selection logic if needed)
genes_d60_alveolar <- top_d60_alveolar5$gene

# Seurat analysis pipeline (adapted to use genes_d60_alveolar)
seurat_d60_1_alveolar %>%
  join_features(genes_d60_alveolar, assay = "RNA", slot = "scale.data") %>%  # Use genes_d60_alveolar for joining
  group_by(genes_d60_alveolar, harmony_cluster_8dims_rough) %>%
  summarise(mean_scaled_dsb = mean(.abundance_RNA)) %>%
  # ... (rest of the code remains the same) ...
  ggplot(aes(reorder(genes_d60_alveolar, harmony_cluster_8dims_rough), harmony_cluster_8dims_rough, fill = zscore_mean_scaled_dsb)) +
  geom_tile() +
  scale_fill_viridis() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  ggtitle("mean dsb-normalized protein counts, z-score of cluster averages per marker")

```

```{r}
# Seurat analysis pipeline (adapted to use genes_d60_alveolar)
seurat_obj <- seurat_d60_1_alveolar %>%
...
# Create heatmap
ggplot(seurat_obj, aes(x = harmony_cluster_8dims_rough, y = .feature, fill = zscore_mean_scaled_dsb)) +
  geom_tile() +
  scale_fill_viridis() +
  theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust = 1)) +
  ggtitle("mean dsb-normalized protein counts, z-score of cluster averages per marker") +
  labs(x = "Harmony Cluster", y = "Gene")  # Optional: Add axis labels
```
...
markers_DNA_vs_RNA_filter = markers_DNA_vs_RNA[markers_DNA_vs_RNA$avg_log2FC > 0.5,]
...
# Categorize genes as "UP" or "DOWN" based on avg_log2FC
markers_DNA_vs_RNA$diffexpressed[markers_DNA_vs_RNA$avg_log2FC > 1] <- "UP"
markers_DNA_vs_RNA$diffexpressed[markers_DNA_vs_RNA$avg_log2FC < -1] <- "DOWN"
...
volcano_alveolar_DNAvsRNA <- ggplot(data = markers_DNA_vs_RNA,
  ...
  scale_x_continuous(breaks = seq(floor(min(markers_DNA_vs_RNA$avg_log2FC)),
                                 ceiling(max(markers_DNA_vs_RNA$avg_log2FC)), by = 0.5)) +
  geom_text_repel(max.overlaps = 100)
