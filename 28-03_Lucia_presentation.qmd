...
```{r}
FeaturePlot(obj.v5, feature = "C5ar1", label = T, label.size = 6)
ggsave("obj_v5_c5ar1_improved.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG")

FeaturePlot(obj.v5, features = "Pparg", label = T, label.size = 6)
ggsave("obj_v5_pparg_improved.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG")


FeaturePlot(obj.v5, features = "Mafb", label = T, label.size = 6)
ggsave("obj_v5_mafb_improved.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG")
```
...
```{r}
obj.v5 |>
  join_features(c("Pparg", "C5AR1","Itgax", "SiglecF", "Ccr2", "Ly6c1"),assay="RNA", slot="scale.data")  |> 
group_by(.feature,harmony_cluster_8dims_rough) |>
  summarise(mean_scaled_dsb=mean(.abundance_RNA)) |>
  group_by(.feature) |> 
  mutate(zscore_mean_scaled_dsb=(mean_scaled_dsb-mean(mean_scaled_dsb))/sd(mean_scaled_dsb))|> na.omit() |> 
  ...
  ggplot(aes(.feature,harmony_cluster_8dims_rough, fill=zscore_mean_scaled_dsb)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +ggtitle("mean dsb-normalized protein counts, z-score of cluster averages per marker")
ggsave("RNA_pparg_cd88_itgax_siglecf_ccr2_ly6c_mean dsb-normalized protein counts, z-score of cluster averages per marker.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/seuratOG")
```
...
## 4c) Heatmap cluster 5 pparg

```{r}
obj.v5_onlycluster5 =  subset(obj.v5, idents = c(5))
obj.v5_onlycluster5 |> 
  ...
  mutate(condition=as.character(condition)) |> 
  na.omit() |> 
  join_features("Pparg",assay="RNA", slot="scale.data")  |> 
  group_by(harmony_cluster_8dims_rough) |> 
  mutate(cluster_mean_freq=mean(freq_cluster)) |> 
  mutate(cluster_mean_freq_zscore=(freq_cluster-cluster_mean_freq)/sd(freq_cluster)) |> 
  ggplot(aes(condition,harmony_cluster_8dims_rough, fill=cluster_mean_freq_zscore)) +
  geom_tile()+ 
  ggh4x::facet_nested_wrap(
    vars(day,condition), nrow = 1,drop = TRUE,scales = "free_x"
  )+theme_bw()+ theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1))   + scale_fill_gradient2(  low = "blue",
  mid = "white",
  high = "red")+ggtitle("proportions of cells from a given sample distributed across clusters ",
                        "Ms4a3 classification as sorted")
...
ggsave(filename = "cluster5_pparg.png",path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/seuratOG/",  height = 20, width = 30, units = "cm")
```
...
write_xlsx(markers_all_filtered, path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/all_markers_filtered_alveolarD60_recluster.xlsx")

fullobj_top20 =  select_top_genes(markers_all_filtered, n = 20)
write_xlsx(fullobj_top20, path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/top20_all_markers.xlsx")


```

```{r}
fullobj_top5 =  select_top_genes(markers_all_filtered, n = 5)
genes_fullobj_top5 = fullobj_top5$gene
genes_fullobj_top5

obj.v5 |>
  join_features(c("Wfdc10", "Lipf", "Acaa1b", "Lepr", "Spag11b", "Rpusd3", "Car4", "Gm34907", "Wfdc21", "Stac2", "Gm15056", "Ccl8", "Arg1", "Ms4a14", "C1qa", "F13a1", "Vcan", "Cd177", "Thbs1", "F5", "Cd7", "Sept3", "Cd209a", "Xcr1", "Itgae", "Tgm3", "S1pr5", "Gm16120", "Slc12a2", "Gjb2", "Pbk", "Esco2", "Pimreg", "Nek2", "Pclaf", "Adhfe1", "Ear6", "Serpina3f", "Cfb", "Slc39a2", "Fscn1", "Gm37422", "Cacnb3", "1810064F22Rik", "Ccr7", "Pmp22", "Mmp12", "H2-M2", "Apoe", "Pianp", "S100a9", "Stfa2l1", "Gm5483", "Asprv1", "Il1f9"),assay="RNA", slot="scale.data")  |> 
group_by(.feature,harmony_cluster_8dims_rough) |>
  summarise(mean_scaled_dsb=mean(.abundance_RNA)) |>
  group_by(.feature) |> 
  mutate(zscore_mean_scaled_dsb=(mean_scaled_dsb-mean(mean_scaled_dsb))/sd(mean_scaled_dsb))|> na.omit() |> 
  ...
  ggplot(aes(.feature,harmony_cluster_8dims_rough, fill=zscore_mean_scaled_dsb)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +ggtitle("mean dsb-normalized protein counts, z-score of cluster averages per marker")
ggsave("RNA_seurat_mean_top5_fullobject dsb-normalized protein counts, z-score of cluster averages per marker.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/seuratOG", width = 20)

```

# 6) Reclustering of alveolar macrophages at d60 (clusters 0 1 5 6 7)

## Dimplots for D0 D8 D60

```{r}
seurat_d0 =  obj.v5 %>%  filter(str_detect(virus, "Mock"))
DimPlot(seurat_d0)
ggsave("dimplot_original_D0.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG/")
seurat_d8 = obj.v5 %>% filter(str_detect(day, "d8"))
Idents(seurat_d8) = "seurat_clusters"
DimPlot(seurat_d8)
ggsave("dimplot_original_D8.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG/")
DimPlot(seurat_d60)
ggsave("dimplot_original_D60.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG/")
```

```{r}
seurat_d60d8_1 = obj.v5  %>% filter(!str_detect(day_mock_sample_type, "Mock"))
seurat_d60d8_1_alveolar =  subset(seurat_d60d8_1, idents = c(0,1,5,6,7))
seurat_d60d8_1_alveolar = seurat_d60d8_1_alveolar %>%  filter(!str_detect(day_sample_type_cond_ms4a3_pos_gabbr2, "Gabbr2_pos_Ms4a3_neg"))
DimPlot(seurat_d60d8_1_alveolar, label = T)
...
metadata_column <- "day"  
value_to_check <- "d8"    
value_present <- check_metadata_presence(seurat_d60d8_1_alveolar, metadata_column, value_to_check)
...
```{r, echo=FALSE}
# seurat_d60d8_1_alveolar = SCTransform(seurat_d60d8_1_alveolar, vst.flavor = 'v2')
# seurat_d60d8_1_alveolar <- FindNeighbors(seurat_d60d8_1_alveolar,  assay = "SCT", graph.name = "harmony_cluster_8dims_rough")  
# seurat_d60d8_1_alveolar <- FindClusters(seurat_d60d8_1_alveolar, resolution = 0.25, cluster.name = "harmony_cluster_8dims_rough", graph.name = "harmony_cluster_8dims_rough" )
# seurat_d60d8_1_alveolar <- RunUMAP(seurat_d60d8_1_alveolar, reduction = "integrated.harmony", dims = 1:8, reduction.name = "umap.harmony_8dims_recluster", assay = "SCT")
# seurat_d60d8_1_alveolar@reductions$umap.harmony_8dims_recluster
# DimPlot(seurat_d60d8_1_alveolar, label = TRUE, group.by = "harmony_cluster_8dims_rough_res.0.25")
# ggsave("dimplot_recluster.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar")

```

```{r, echo=FALSE}
seurat_d60d8_1_alveolar = FindVariableFeatures(seurat_d60d8_1_alveolar)
seurat_d60d8_1_alveolar = NormalizeData(seurat_d60d8_1_alveolar)
seurat_d60d8_1_alveolar = ScaleData(seurat_d60d8_1_alveolar)
seurat_d60d8_1_alveolar <- FindNeighbors(seurat_d60d8_1_alveolar, reduction = "integrated.harmony", dims = 1:8)
seurat_d60d8_1_alveolar <- FindClusters(seurat_d60d8_1_alveolar, cluster.name = "harmony_cluster_8dims_rough", resolution = 0.25)

seurat_d60d8_1_alveolar <- RunUMAP(seurat_d60d8_1_alveolar, reduction = "integrated.harmony", dims = 1:8, reduction.name = "umap.harmony_8dims_recluster")
seurat_d60d8_1_alveolar|> DimPlot(group.by = "harmony_cluster_8dims_rough", label=T)
ggsave("dimplot_recluster.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar")
```
...
```{r}
DimPlot_scCustom(seurat_d60d8_1_alveolar, split.by = "condition", colors_use = c("purple", "cyan", "lightgreen", "red", "orange", "blue", "pink"))
...
```{r}
DimPlot_scCustom(seurat_d60d8_1_alveolar, split.by = "condition", group.by = "sample_type",colors_use = c("blue", "orange"))
ggsave("dimplot_virusGroupedTissue.png", width = 40, height = 30, units = "cm", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar")
```

## 6e) dimplots for Ms4a3 + -

```{r}
DimPlot_scCustom(seurat_d60d8_1_alveolar, split.by = "condition", group.by = "sampletag_Ms4a3", colors_use = c("darkgrey", "red") )
ggsave("dimplot_virusGroupedMs4a3.png", width = 40, height = 30, units = "cm", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar")
```
...
seurat_d60d8_1_alveolar@meta.data$sampletag_name_factor = factor(seurat_d60d8_1_alveolar@meta.data$sampletag_name, levels = sort(unique(seurat_d60d8_1_alveolar$sampletag_name)))

Idents(seurat_d60d8_1_alveolar) = "sampletag_name_factor"
DimPlot_scCustom(seurat_d60d8_1_alveolar, split.by = "sampletag_name_factor", num_columns = 10, colors_use = c("red", "orange", "black", "darkgrey", "blue", "cyan", "green", "lightgreen", "purple", "pink"), label = F)
ggsave("dimplot_virusGroupedMs4a3_virus.png", width = 49 , path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar")
```
...
dimplot_Ms4a3 = DimPlot_scCustom(seurat_d60d8_1_alveolar, group.by = "sampletag_Ms4a3",  colors_use = c("darkgrey", "red"))
dimplot_Ms4a3
ggsave("umap_Ms4a3.png", bg = 'white',  path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar//")
...
pallet = viridis(n = 10, option = "D")
Idents(seurat_d60d8_1_alveolar) = "sampletag_name"
DimPlot_All_Samples(seurat_d60d8_1_alveolar, meta_data_column = 'sampletag_name', colors_use = pallet, num_columns = 10)

ggsave("umap_Ms4a3_virus.png", bg = 'white', width = 49 , path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar/")
```

## 6i) AbSeq Featureplots of H2-ab1 Itgax Pparg SiglecF and CD274 for Ms4a3 + -

```{r}
Idents(seurat_d60d8_1_alveolar) = "seurat_clusters"
DefaultAssay(seurat_d60d8_1_alveolar) = "adt"
...
for (i in markers_am) {
  plot <- FeaturePlot_scCustom(seurat_d60d8_1_alveolar,  features = i, label = FALSE, label.size = 5,  split.by = "sampletag_Ms4a3", colors_use = pallet, label_feature_yaxis = FALSE )
...
create_and_save_plot <- function(gene) {
  plot <- FeaturePlot_scCustom(seurat_d60d8_1_alveolar, 
                              features = gene, 
                              label = TRUE, 
                              label.size = 5, 
  ...
  ggsave(plot = plot, 
         ...
         height = 20, 
         units = "cm", 
         path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/Alveolar")
}
...
```{r}
DefaultAssay(seurat_d60d8_1_alveolar) = "RNA"
...
markers_am = unique(genes_AM)
for (i in markers_am) {
  plot <- FeaturePlot_scCustom(seurat_d60d8_1_alveolar,   features = i, label = TRUE, label.size = 5,  split.by = "sampletag_Ms4a3", colors_use = pallet, label_feature_yaxis = FALSE)
...
markers_am = unique(genes_AM)
for (i in markers_am) {
  plot <- FeaturePlot_scCustom(seurat_d60d8_1_alveolar,   features = i, label = TRUE, label.size = 5,  split.by = "sample_type", colors_use = pallet, label_feature_yaxis = FALSE)
  umap_am_alveolar_d60[[i]] <- plot
}
# Arrange plots using ggarrange
ggarrange(plotlist = umap_am_alveolar_d60, nrow = 5)
...
```{r}
seurat_d60d8_1_alveolar_bal = seurat_d60d8_1_alveolar %>% filter(str_detect(sample_type, "bal"))
seurat_d60d8_1_alveolar_bal = FindVariableFeatures(seurat_d60d8_1_alveolar_bal)
seurat_d60d8_1_alveolar_bal = NormalizeData(seurat_d60d8_1_alveolar_bal)
seurat_d60d8_1_alveolar_bal = ScaleData(seurat_d60d8_1_alveolar_bal)
DimPlot(seurat_d60d8_1_alveolar_bal, reduction = "pca", group.by = "condition")
ggsave("pcaplot_alveolar_bal_virus.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/PCA")
DimPlot(seurat_d60d8_1_alveolar_bal, reduction = "pca", group.by = "sampletag_Ms4a3")
ggsave("pcaplot_alveolar_bal_Ms4a3.png", path =  "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/PCA")
DimPlot(seurat_d60d8_1_alveolar_bal, reduction = "pca", group.by = "sampletag_name")
...
value_to_check = "lung"
metadata_column = "day_sample_type_cond_ms4a3_pos_gabbr2"
value_present <- check_metadata_presence(seurat_d60d8_1_alveolar_bal, metadata_column, value_to_check)
...
value_to_check = "bal"
metadata_column = "day_sample_type_cond_ms4a3_pos_gabbr2"
value_present <- check_metadata_presence(seurat_d60d8_1_alveolar_lung, metadata_column, value_to_check)
...
```{r}
seurat_d60d8_1_alveolar_lung = seurat_d60d8_1_alveolar %>% filter(str_detect(sample_type, "lung"))
seurat_d60d8_1_alveolar_lung = FindVariableFeatures(seurat_d60d8_1_alveolar_lung)
seurat_d60d8_1_alveolar_lung = NormalizeData(seurat_d60d8_1_alveolar_lung)
seurat_d60d8_1_alveolar_lung = ScaleData(seurat_d60d8_1_alveolar_lung)

DimPlot(seurat_d60d8_1_alveolar_lung, reduction = "pca", group.by = "condition")
ggsave("pcaplot_alveolar_lung_condition.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/PCA")
DimPlot(seurat_d60d8_1_alveolar_lung, reduction = "pca", group.by = "sampletag_Ms4a3")
ggsave("pcaplot_alveolar_lung_Ms4a3.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/PCA")
DimPlot(seurat_d60d8_1_alveolar_lung, reduction = "pca", group.by = "sampletag_name")
ggsave("pcaplot_alveolar_lung_condition_Ms4a3.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/PCA")
```

## 6p) PC plot showing Ms4a3 and sample type

```{r}
DimPlot(seurat_d60d8_1_alveolar, reduction = "pca", group.by = "sampletag_name", split.by = "sample_type")
ggsave("pcaplot_alveolar_bal_lung_condition_Ms4a3.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/PCA")
```

## 6q) heatmap 4b but for seurat_d60d8_1_alveolar
...
ggsave("cd11c_abseq.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG/" )
```
...
plot_siglecf + scale_fill_continuous(limits = c(0, 15)) +  # Remove extra space here
  scale_color_continuous(limits = c(0, 15)) +
  scale_size_continuous(limits = c(0, 15))
ggsave("siglecf_abseq.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/UMAP/seuratOG/")
...
```{r, echo=FALSE}
markers_d60d8_alveolar <- FindAllMarkers(seurat_d60d8_1_alveolar,
                          assay = "RNA", 
                          logfc.threshold = 0.5,
                          test.use = "wilcox", 
                          min.pct = 0.2)
write_xlsx(markers_d60d8_alveolar, path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_d60_alveolar.xlsx")
```

```{r}
markers_d60d8_alveolar_filtered = markers_d60d8_alveolar[markers_d60d8_alveolar$avg_log2FC > 1,]
nonsignficant_p_adj_d60_alveolar <- which(markers_d60d8_alveolar_filtered$p_val_adj <= 0.05)
markers_d60d8_alveolar_filtered <- markers_d60d8_alveolar_filtered[nonsignficant_p_adj_d60_alveolar, ]
write_xlsx(markers_d60d8_alveolar_filtered, path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_d60d8_alveolar_filtered.xlsx")
```

```{r}
#select top 20 genes from each cluster

markers_d60d8_alveolar_filtered = data.frame(markers_d60d8_alveolar_filtered)

top_d60d8_alveolar <- select_top_genes(markers_d60d8_alveolar_filtered, n = 20)
write_xlsx(top_d60d8_alveolar,path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/top20_per_cluster_markers_d60d8_alveolar.xlsx")
```

```{r}
top_d60d8_alveolar5 <- select_top_genes(markers_d60d8_alveolar_filtered, n = 5)

genes_d60d8_alveolar = top_d60d8_alveolar5$gene
genes_d60d8_alveolar = as.factor(genes_d60d8_alveolar)

# Library loading (assuming libraries are already loaded)
# library(ggplot2)
# library(viridis)
# library(dplyr)
# library(Seurat)
...
# Seurat analysis pipeline (adapted to use genes_d60_alveolar)
seurat_d60d8_1_alveolar %>% as_tibble() %>% 
  join_features(genes_d60d8_alveolar, assay = "RNA", slot = "scale.data") %>%  # Use genes_d60_alveolar for joining
  group_by(genes_d60d8_alveolar,harmony_cluster_8dims_rough) |>
  summarise(mean_scaled_dsb=mean(.abundance_RNA)) |>
  group_by(genes_d60d8_alveolar) |> 
  mutate(zscore_mean_scaled_dsb=(mean_scaled_dsb-mean(mean_scaled_dsb))/sd(mean_scaled_dsb))|> 
  mutate(zscore_mean_scaled_dsb=ifelse(zscore_mean_scaled_dsb>2,2,
                                       ifelse(zscore_mean_scaled_dsb<(-2),-2,zscore_mean_scaled_dsb))) |> 
  ggplot(aes(.feature,harmony_cluster_8dims_rough, fill=zscore_mean_scaled_dsb)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +ggtitle("mean dsb-normalized protein counts, z-score of cluster averages per marker")
ggsave("RNA_seurat_clusters_top5_alveolar dsb-normalized protein counts, z-score of cluster averages per marker.svg", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/Alveolar/", width = 20)

```

# do this like point 6g
...
genes_d60_d8_cluster = as.factor(c("Esco2", "Cdca7", "Cxcl2", "Nos2", "Tmem26", "Lipf", "Pbk", "Ung", "Hilpda", "Ifit1", "Ifitm6", "Sema3e", "Nek2", "Mcm6", "Nr4a3", "Cxcl9", "Gjb2", "Sema3e", "Ska1", "Mcm2", "Cxcr1", "Ifit2", "Adgre4", "Colec12", "Pimreg", "Fignl1", "Il1a", "Serpina3f", "Pglyrp1", "Oaf", "Plac8", "Marco", "Cd93", "Lrg1", "Mmp8", "Spp1", "S100a6"))
seurat_d60d8_1_alveolar |>
  join_features(as.factor(c("Esco2", "Cdca7", "Cxcl2", "Nos2", "Tmem26", "Lipf", "Pbk", "Ung", "Hilpda", "Ifit1", "Ifitm6", "Sema3e", "Nek2", "Mcm6", "Nr4a3", "Cxcl9", "Gjb2", "Sema3e", "Ska1", "Mcm2", "Cxcr1", "Ifit2", "Adgre4", "Colec12", "Pimreg", "Fignl1", "Il1a", "Serpina3f", "Pglyrp1", "Oaf", "Plac8", "Marco", "Cd93", "Lrg1", "Mmp8", "Spp1", "S100a6"
...
group_by(.feature,harmony_cluster_8dims_rough) |>
  ...
  ggplot(aes(.feature,harmony_cluster_8dims_rough, fill=zscore_mean_scaled_dsb)) +
  geom_tile()+ scale_fill_viridis() + theme(axis.text.x = element_text(angle = 45, vjust = 1, hjust=1)) +ggtitle("mean dsb-normalized protein counts, z-score of cluster averages per marker")
ggsave("RNA_markers_alveolar_mean dsb-normalized protein counts, z-score of cluster averages per marker.png", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/Alveolar/", width = 15)
ggsave("RNA_markers_alveolar_mean dsb-normalized protein counts, z-score of cluster averages per marker.svg", path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/Alveolar/", width = 15)
filename <- "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/Alveolar/RNA_markers_alveolar_mean.eps"
postscript(filename, width = 15)

```

```{r}
seurat_d60d8_1_alveolar |>
  join_features(genes_d60_d8_cluster,assay="RNA", slot="scale.data")  |> 
group_by(.feature,harmony_cluster_8dims_rough) |>
  summarise(mean_scaled_dsb=mean(.abundance_RNA)) |>
  group_by(.feature) |> 
  mutate(zscore_mean_scaled_dsb=(mean_scaled_dsb-mean(mean_scaled_dsb))/sd(mean_scaled_dsb))|> na.omit() |> 
...
```{r}
DefaultAssay(seurat_d60d8_1_alveolar_noMock) = "RNA"
DefaultAssay(seurat_d60d8_1_alveolar_Mock) = "RNA"

seurat_d60d8_1_alveolar_noMock = seurat_d60d8_1_alveolar %>% filter(!str_detect(condition, "Mock"))
seurat_d60d8_1_alveolar_noMock = FindVariableFeatures(seurat_d60d8_1_alveolar_noMock, loess.span = 0.8)
seurat_d60d8_1_alveolar_noMock = ScaleData(seurat_d60d8_1_alveolar_noMock)

seurat_d60d8_1_alveolar_Mock = seurat_d60d8_1_alveolar %>% filter(str_detect(condition, "Mock"))
seurat_d60d8_1_alveolar_Mock = FindVariableFeatures(seurat_d60d8_1_alveolar_Mock)
seurat_d60d8_1_alveolar_Mock = ScaleData(seurat_d60d8_1_alveolar_Mock)
```

```{r}

Idents(seurat_d60d8_1_alveolar_noMock) = "sampletag_Ms4a3"
markers_alveolar_nomock_pos<- FindMarkers(seurat_d60d8_1_alveolar_noMock ,
                                 ident.1 =  "Ms4a3_pos",
                                 ident.2 =  "Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_nomock_neg <- FindMarkers(seurat_d60d8_1_alveolar_noMock ,
                                 ident.1 =  "Ms4a3_neg",
                                 ident.2 =  "Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
...
```{r}
Idents(seurat_d60d8_1_alveolar_Mock) = "sampletag_Ms4a3"
markers_alveolar_mock_pos<- FindMarkers(seurat_d60d8_1_alveolar_Mock ,
                                 ident.1 =  "Ms4a3_pos",
                                 ident.2 =  "Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_mock_neg <- FindMarkers(seurat_d60d8_1_alveolar_Mock ,
                                 ident.1 =  "Ms4a3_neg",
                                 ident.2 =  "Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
...
Idents(seurat_d60d8_1_alveolar_noMock) = "condition"

seurat_d60d8_1_alveolar_noMock_MuHV4 = subset(seurat_d60d8_1_alveolar_noMock, idents = "MuHV4")
DefaultAssay(seurat_d60d8_1_alveolar_noMock_MuHV4) = "RNA"
seurat_d60d8_1_alveolar_noMock_MuHV4 = FindVariableFeatures(seurat_d60d8_1_alveolar_noMock_MuHV4)
seurat_d60d8_1_alveolar_noMock_MuHV4 = ScaleData(seurat_d60d8_1_alveolar_noMock_MuHV4)


seurat_d60d8_1_alveolar_noMock_PR8 = subset(seurat_d60d8_1_alveolar_noMock, idents = "PR8")
DefaultAssay(seurat_d60d8_1_alveolar_noMock_PR8) = "RNA"
seurat_d60d8_1_alveolar_noMock_PR8 = FindVariableFeatures(seurat_d60d8_1_alveolar_noMock_PR8)
seurat_d60d8_1_alveolar_noMock_PR8 = ScaleData(seurat_d60d8_1_alveolar_noMock_PR8)


seurat_d60d8_1_alveolar_noMock_MAV1 = subset(seurat_d60d8_1_alveolar_noMock, idents = "MAV1")
DefaultAssay(seurat_d60d8_1_alveolar_noMock_MAV1) = "RNA"
seurat_d60d8_1_alveolar_noMock_MAV1 = FindVariableFeatures(seurat_d60d8_1_alveolar_noMock_MAV1)
seurat_d60d8_1_alveolar_noMock_MAV1 = ScaleData(seurat_d60d8_1_alveolar_noMock_MAV1)


seurat_d60d8_1_alveolar_noMock_PVM = subset(seurat_d60d8_1_alveolar_noMock, idents = "PVM")
DefaultAssay(seurat_d60d8_1_alveolar_noMock_PVM) = "RNA"
seurat_d60d8_1_alveolar_noMock_PVM = FindVariableFeatures(seurat_d60d8_1_alveolar_noMock_PVM)
seurat_d60d8_1_alveolar_noMock_PVM = ScaleData(seurat_d60d8_1_alveolar_noMock_PVM)        
...
```{r}
Idents(seurat_d60d8_1_alveolar_noMock_MuHV4) = "sampletag_Ms4a3"
markers_alveolar_nomock_pos_MuHV4<- FindMarkers(seurat_d60d8_1_alveolar_noMock_MuHV4 ,
                                 ident.1 =  "Ms4a3_pos",
                                 ident.2 =  "Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_nomock_neg_MuHV4 <- FindMarkers(seurat_d60d8_1_alveolar_noMock_MuHV4 ,
                                 ident.1 =  "Ms4a3_neg",
                                 ident.2 =  "Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
...
```{r}
Idents(seurat_d60d8_1_alveolar_noMock_PR8) = "sampletag_Ms4a3"
markers_alveolar_nomock_pos_PR8<- FindMarkers(seurat_d60d8_1_alveolar_noMock_PR8 ,
                                 ident.1 =  "Ms4a3_pos",
                                 ident.2 =  "Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_nomock_neg_PR8 <- FindMarkers(seurat_d60d8_1_alveolar_noMock_PR8 ,
                                 ident.1 =  "Ms4a3_neg",
                                 ident.2 =  "Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
...
```{r}
Idents(seurat_d60d8_1_alveolar_noMock_PVM) = "sampletag_Ms4a3"
markers_alveolar_nomock_pos_PVM<- FindMarkers(seurat_d60d8_1_alveolar_noMock_PVM ,
                                 ident.1 =  "Ms4a3_pos",
                                 ident.2 =  "Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_nomock_neg_PVM <- FindMarkers(seurat_d60d8_1_alveolar_noMock_PVM ,
                                 ident.1 =  "Ms4a3_neg",
                                 ident.2 =  "Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
...
```{r}
Idents(seurat_d60d8_1_alveolar_noMock_MAV1) = "sampletag_Ms4a3"
markers_alveolar_nomock_pos_MAV1<- FindMarkers(seurat_d60d8_1_alveolar_noMock_MAV1 ,
                                 ident.1 =  "Ms4a3_pos",
                                 ident.2 =  "Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_nomock_neg_MAV1 <- FindMarkers(seurat_d60d8_1_alveolar_noMock_PR8 ,
                                 ident.1 =  "Ms4a3_neg",
                                 ident.2 =  "Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
...
# Categorize genes as "UP" or "DOWN" based on avg_log2FC
markers_alveolar_nomock_pos_MAV1$diffexpressed[markers_alveolar_nomock_pos_MAV1$avg_log2FC > 0.5] <- "UP"
markers_alveolar_nomock_pos_MAV1$diffexpressed[markers_alveolar_nomock_pos_MAV1$avg_log2FC < -0.5] <- "DOWN"
...
```{r}
Idents(seurat_d60d8_1_alveolar) = "condition"
markers_alveolar_MuHV4<- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "MuHV4",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_PR8 <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "PR8",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_PVM <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "PVM",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_MAV1 <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "MAV1",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_Mock <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "Mock",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
...
# 10) DEGs comparing each infection to all the groups including Ms4a3, including Mock.

```{r}
Idents(seurat_d60d8_1_alveolar) = "sampletag_name"
markers_alveolar_MuHV4n<- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "MuHV4_Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_MuHV4p<- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "MuHV4_Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_PR8n <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "PR8_Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_PR8p <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "PR8_Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_PVMn <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "PVM_Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_PVMp <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "PVM_Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_MAV1n <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "MAV1_Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_MAV1p <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "MAV1_Ms4a3_pos",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_Mockn <- FindMarkers(seurat_d60d8_1_alveolar,
                                 ident.1 =  "Mock_Ms4a3_neg",
                                 test.use = "wilcox", 
                                 assay = "RNA",
                                 min.pct = 0.2) |> as_tibble(rownames="gene")
markers_alveolar_Mockp <- FindMarkers(seurat_d60d8_1_alveolar,
...
# 19)GSEA using WebgestaltR

#read in all excel files from 10

```{r}
#MAV1
MAV1_positive = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_MAV1p_filtered.xlsx")

MAV1_positive_up <- MAV1_positive[MAV1_positive$avg_log2FC > 1,]
MAV1_positive_down <- MAV1_positive[MAV1_positive$avg_log2FC < -1,]
MAV1_positive_up_genes = MAV1_positive_up$gene
MAV1_positive_down_genes = MAV1_positive_down$gene


MAV1_negative = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_MAV1n_filtered.xlsx")

MAV1_negative_up <- MAV1_negative[MAV1_negative$avg_log2FC > 1,]
MAV1_negative_down <- MAV1_negative[MAV1_negative$avg_log2FC < -1,]
MAV1_negative_up_genes = MAV1_negative_up$gene
MAV1_negative_down_genes = MAV1_negative_down$gene


#Mock

Mock_positive = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_Mockp_filtered.xlsx")

Mock_positive_up <- Mock_positive[Mock_positive$avg_log2FC > 1,]
Mock_positive_down <- Mock_positive[Mock_positive$avg_log2FC < -1,]
mock_positive_up_genes = Mock_positive_up$gene
mock_positive_down_genes = Mock_positive_down$gene
Mock_positive_down <- Mock_positive[Mock_positive$avg_log2FC < -1, c("gene", "avg_log2FC")]
mock_positive_down_genes <- Mock_positive_down[, 1]  
mock_positive_down_fc_values <- Mock_positive_down[, 2]  

Mock_negative = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_Mockn_filtered.xlsx")

Mock_negative_up <- Mock_negative[Mock_negative$avg_log2FC > 1,]
Mock_negative_down <- Mock_negative[Mock_negative$avg_log2FC < -1,]
mock_negative_up_genes = Mock_negative_up$gene
mock_negative_down_genes = Mock_negative_down$gene


#MuHV4

MuHV4_positive = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_MuHV4p_filtered.xlsx")

MuHV4_positive_up <- MuHV4_positive[MuHV4_positive$avg_log2FC > 1,]
MuHV4_positive_down <- MuHV4_positive[MuHV4_positive$avg_log2FC < -1,]
MuHV4_positive_up_genes = MuHV4_positive_up$gene
MuHV4_positive_down_genes = MuHV4_positive_down$gene

MuHV4_negative = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_MuHV4n_filtered.xlsx")

MuHV4_negative_up <- MuHV4_negative[MuHV4_negative$avg_log2FC > 1,]
MuHV4_negative_down <- MuHV4_negative[MuHV4_negative$avg_log2FC < -1,]
MuHV4_negative_up_genes = MuHV4_negative_up$gene
MuHV4_negative_down_genes = MuHV4_negative_down$gene

#PR8
PR8_positive = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_PR8p_filtered.xlsx")

PR8_positive_up <- PR8_positive[PR8_positive$avg_log2FC > 1,]
PR8_positive_down <- PR8_positive[PR8_positive$avg_log2FC < -1,]
PR8_positive_up_genes = PR8_positive_up$gene
PR8_positive_down_genes = PR8_positive_down$gene

PR8_negative = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_PR8n_filtered.xlsx")

PR8_negative_up <- PR8_negative[PR8_negative$avg_log2FC > 1,]
PR8_negative_down <- PR8_negative[PR8_negative$avg_log2FC < -1,]
PR8_negative_up_genes = PR8_negative_up$gene
PR8_negative_down_genes = PR8_negative_down$gene



#PVM
PVM_positive = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_PVMp_filtered.xlsx")

PVM_positive_up <- PVM_positive[PVM_positive$avg_log2FC > 1,]
PVM_positive_down <- PVM_positive[PVM_positive$avg_log2FC < -1,]
PVM_positive_up_genes = PVM_positive_up$gene
PVM_positive_down_genes = PVM_positive_down$gene

PVM_negative = read_xlsx("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_PVMn_filtered.xlsx")

PVM_negative_up <- PVM_negative[PVM_negative$avg_log2FC > 1,]
PVM_negative_down <- PVM_negative[PVM_negative$avg_log2FC < -1,]
PVM_negative_up_genes = PVM_negative_up$gene
PVM_negative_down_genes = PVM_negative_down$gene



objects <- ls()

# Filter objects ending with "genes"
gene_objects <- objects[grep("genes$", objects)]

# Loop through each gene object and write its contents to a text file
for (gene_obj in gene_objects) {
  # Extract the vector from the environment
  gene_vector <- get(gene_obj)
  
  # Define the file name
  file_name <- paste(gene_obj, ".txt", sep="")
  
  # Write the vector to a text file
  write.table(gene_vector, file=file_name, row.names=FALSE, col.names=FALSE)
}
```

```{r}
# Function to process viral condition data
process_virus_data <- function(virus_name, condition) {
  # Read data from excel file
  data <- read_xlsx(paste0("../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/h/markers_alveolar_", virus_name, condition, "_filtered.xlsx"))
  
  # Filter data for up-regulated genes
  data_up <- data[data$avg_log2FC > 1,]
  up_genes <- data_up$gene
  up_fc_values <- data_up$avg_log2FC
  
  # Filter data for down-regulated genes (including avg_log2FC)
  data_down <- data[data$avg_log2FC < -1, c("gene", "avg_log2FC")]
  down_genes <- data_down[, 1]
  down_fc_values <- data_down[, 2]
  
  # Combine gene names and fold change values for down-regulated genes
  down_regulated <- cbind(down_genes, down_fc_values)
  
  # Return results as a list
  return(list(up_genes = up_genes, down_regulated = down_regulated))
}

# Process data for each virus condition
virus_data <- list()
virus_data[["MAV1n"]] <- process_virus_data("MAV1", "n")
virus_data[["Mock"]] <- process_virus_data("Mock")
virus_data[["MuHV4"]] <- process_virus_data("MuHV4")
virus_data[["PR8"]] <- process_virus_data("PR8")
virus_data[["PVM"]] <- process_virus_data("PVM")

# Write gene data to text files
for (virus, data in virus_data) {
  # Define file names
  up_file <- paste0(virus, "_up_genes.txt")
  down_file <- paste0(virus, "_down_regulated.txt")
  
  # Write gene names for up-regulated genes
  write.table(data$up_genes, file = up_file, row.names = FALSE, col.names = FALSE)
  
  # Write combined data (gene names and fold change) for down-regulated genes
  write.table(data$down_regulated, file = down_file, row.names = FALSE, col.names = FALSE)
}
```

# markers_alveolar_MuHV4vsMock_filtered = markers_alveolar_MuHV4vsMock\[abs(markers_alveolar_MuHV4vsMock\['avg_log2FC'\]) == 0.5\] 20) Differential expression DNA viruses RNA viuses
...
```{r}
markers_DNA_vs_mock_filter = markers_DNA_vs_mock[markers_DNA_vs_mock$avg_log2FC > 1,]
...
write_xlsx(markers_DNA_vs_mock_filter, "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_DNA_vs_mock_filter.xlsx")
write_xlsx(markers_DNA_vs_mock, "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_DNA_vs_mock.xlsx")



markers_RNA_vs_mock_filter = markers_RNA_vs_mock[markers_RNA_vs_mock$avg_log2FC > 1,]
nonsignficant_p_adj_RNA <- which(markers_RNA_vs_mock$p_val_adj <= 0.05)
markers_RNA_vs_mock_filter <- markers_DNA_vs_mock_filter[nonsignficant_p_adj_RNA, ]

write_xlsx(markers_RNA_vs_mock_filter, "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_RNA_vs_mock_filter.xlsx")
write_xlsx(markers_RNA_vs_mock, "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_RNA_vs_mock.xlsx")


markers_DNA_vs_RNA_filter = markers_DNA_vs_RNA[markers_DNA_vs_RNA$avg_log2FC > 0.75,]
nonsignficant_p_adj_DNA <- which(markers_DNA_vs_RNA$p_val_adj <= 0.05)
markers_DNA_vs_RNA_filter <- markers_DNA_vs_RNA_filter[nonsignficant_p_adj_DNA, ]

write_xlsx(markers_DNA_vs_RNA_filter, "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_DNA_vs_RNA_filter.xlsx")
write_xlsx(markers_DNA_vs_RNA, "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/markers_DNA_vs_RNA.xlsx")
```

```{r}
# Assign "NO" to diffexpressed column
markers_DNA_vs_RNA$diffexpressed <- "NO"

# Categorize genes as "UP" or "DOWN" based on avg_log2FC
markers_DNA_vs_RNA$diffexpressed[markers_DNA_vs_RNA$avg_log2FC > 0.75] <- "UP"
markers_DNA_vs_RNA$diffexpressed[markers_DNA_vs_RNA$avg_log2FC < -0.75] <- "DOWN"

# Add a column for labels
markers_DNA_vs_RNA$delabel <- NA

# Assign gene names as labels for differentially expressed genes
markers_DNA_vs_RNA$delabel[markers_DNA_vs_RNA$diffexpressed != "NO"] <-
  markers_DNA_vs_RNA$gene[markers_DNA_vs_RNA$diffexpressed != "NO"]  # Ensure consistent variable names




volcano_alveolar_DNAvsRNA <- ggplot(data = markers_DNA_vs_RNA,
       aes(x = avg_log2FC, y = -log10(p_val_adj), col = diffexpressed, label = delabel)) +
  geom_point() +
  theme_minimal() +
  scale_x_continuous(breaks = seq(floor(min(markers_DNA_vs_RNA$avg_log2FC)),
                                   ceiling(max(markers_DNA_vs_RNA$avg_log2FC)), by = 0.5)) +  geom_text_repel(max.overlaps = 100) + ylim(0,500)

volcano_alveolar_DNAvsRNA

# Save the plot
ggsave("volcano_alveolar_DNAvsRNA.png",
       width = 45, height = 30, units = "cm",
       path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/", bg = 'white')
filename <- "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/Alveolar/DNAvsRNvolcano.eps"
postscript(filename, width = 45, height = 30)
```

```{r}
# Assign "NO" to diffexpressed column
markers_DNA_vs_mock$diffexpressed <- "NO"

# Categorize genes as "UP" or "DOWN" based on avg_log2FC
markers_DNA_vs_mock$diffexpressed[markers_DNA_vs_mock$avg_log2FC > 1] <- "UP"
markers_DNA_vs_mock$diffexpressed[markers_DNA_vs_mock$avg_log2FC < -1] <- "DOWN"

# Add a column for labels
markers_DNA_vs_mock$delabel <- NA

# Assign gene names as labels for differentially expressed genes
markers_DNA_vs_mock$delabel[markers_DNA_vs_mock$diffexpressed != "NO"] <-
  markers_DNA_vs_mock$gene[markers_DNA_vs_mock$diffexpressed != "NO"]  # Ensure consistent variable names




volcano_alveolar_DNAvsmock <- ggplot(data = markers_DNA_vs_mock,
       aes(x = avg_log2FC, y = -log10(p_val_adj), col = diffexpressed, label = delabel)) +
  geom_point() +
  theme_minimal() +
  scale_x_continuous(breaks = seq(floor(min(markers_DNA_vs_mock$avg_log2FC)),
                                 ceiling(max(markers_DNA_vs_mock$avg_log2FC)), by = 0.5)) +
  geom_text_repel(max.overlaps = 100)
...
# Save the plot
# Save the plot
ggsave("volcano_alveolar_DNAvsmock.png",
       width = 45, height = 20, units = "cm",
       path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/", bg = "white")
```
...
```{r}
# Assign "NO" to diffexpressed column
markers_RNA_vs_mock$diffexpressed <- "NO"

# Categorize genes as "UP" or "DOWN" based on avg_log2FC
markers_RNA_vs_mock$diffexpressed[markers_RNA_vs_mock$avg_log2FC > 1] <- "UP"
markers_RNA_vs_mock$diffexpressed[markers_RNA_vs_mock$avg_log2FC < -1] <- "DOWN"

# Add a column for labels
markers_RNA_vs_mock$delabel <- NA

# Assign gene names as labels for differentially expressed genes
markers_RNA_vs_mock$delabel[markers_RNA_vs_mock$diffexpressed != "NO"] <-
  markers_RNA_vs_mock$gene[markers_RNA_vs_mock$diffexpressed != "NO"]  # Ensure consistent variable names




volcano_alveolar_RNAvsmock <- ggplot(data = markers_RNA_vs_mock,
       aes(x = avg_log2FC, y = -log10(p_val_adj), col = diffexpressed, label = delabel)) +
  geom_point() +
  theme_minimal() +
  scale_x_continuous(breaks = seq(floor(min(markers_RNA_vs_mock$avg_log2FC)),
                                 ceiling(max(markers_RNA_vs_mock$avg_log2FC)), by = 0.5)) +  geom_text_repel(max.overlaps = 100)


volcano_alveolar_RNAvsmock

# Save the plot
ggsave("volcano_alveolar_RNAvsmock.png",
       width = 45, height = 20, units = "cm",
       path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/DEA/", bg = 'white')
```

# 21) Heatmap markers daniel

### 'clusters_gene_set.xlsx' for full object
