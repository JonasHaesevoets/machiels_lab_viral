...
```{r}
DimPlot_scCustom(seurat_d60_1_alveolar, split.by = "condition", group.by = "sample_type")
...
seurat_d60_1_alveolar |>  
  ...
  dplyr::group_by(sample_type, condition,sampletag_Ms4a3,harmony_cluster_8dims_rough) |> 
  dplyr::count(sample_type, condition,sampletag_Ms4a3,harmony_cluster_8dims_rough, .drop = FALSE) |> 
  mutate(sample=paste(sample_type,condition,sampletag_Ms4a3)) |> 
  filter(!(condition %in% c("PR8","MuHV4", "PVM", "MAV1"))) |> 
  mutate(freq_cluster=n/sum(n)) |> 
  mutate(condition=as.character(condition)) |> 
  na.omit() |>  
  group_by(harmony_cluster_8dims_rough) |> 
...
ggsave(filename = "proportions of cells from a given sample distributed across clusters.png",
       path = "../../Documents/machiels_lab_viral/28-03_Lucia_presentation_files/heatmap/seuratOG",
