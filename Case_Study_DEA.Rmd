...
counts <- GetAssayData(seurat_d60, assay = "RNA")
counts <- counts[-(which(rownames(counts) %in% c("Ddx3y", "Eif2s3y", "Kdm5d", "Xist", "Uty"))),]
seurat_d60 <- subset(seurat_d60, features = rownames(counts))
...
```{r}
MAV1vsMock_ordered = prepare_rnk(data = markers_MAV1vsMock)
...
```{r}
MuHV4vsMock_ordered = prepare_rnk(markers_MuHV4vsMock) 
...
```{r}
PR8vsMock_ordered = prepare_rnk(markers_PR8vsMock)
...
```{r}
PVMvsMock_ordered = prepare_rnk(markers_PVMvsMock)
