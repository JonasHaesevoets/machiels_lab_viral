...
output = "C:/Users/Jonas/Desktop/Analysis_Lucia/R_Projects/machiels_lab_viral-main/output/deseq/"


### split the object in 2 based on the experiment i.e. the amount of days
obj_experiment_list = SplitObject(obj, split.by = "day")

obj_experiment_1 = obj_experiment_list$d60
obj_experiment_2 = obj_experiment_list$d8

DefaultAssay(obj_experiment_1) <- "RNA" ## make sure that the right assay is used
...
keep_experiment1 <- obj_sex_experiment1 |> filter(sex_classifier %in% c("male","female")) |> select(.cell,sex_classifier)
...
keep_experiment2 <- obj_sex_experiment2 |> filter(sex_classifier %in% c("male","female")) |> select(.cell,sex_classifier)
...
expression_matrix_experiment1 <- as.matrix(obj_sex_corrected_experiment1_sums %>% select(-gene))   #|> #select(-contains("smaller"))
## converts to expression matrix storing expression values for each sample
rownames(expression_matrix_experiment1) <- obj_sex_corrected_experiment1_sums %>% pull(gene)
...
dds_full_experiment1_tissue = DESeqDataSetFromMatrix(expression_matrix_experiment1,
                                   ...
                                   design= ~ tissue + gender + virus + Ms4a3)
